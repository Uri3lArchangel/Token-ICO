...
function Header(props) {
  ...
  return (
    ...
    <div className='mt-6 space-y-8 section1Text'>
      ...
      <Image src={Horse} className=' rounded-full px-2' layout='intrinsic'/>
      </div>
   ...
   <h1 className='text-center my-4 text-3xl font-bold text-white'>Price:0.01ETH </h1>
    <input onChange={props.change} min={1} max={props.icoBalance} type="number" className='block text-center mx-auto rounded-full w-full h-8 px-2 outline-none' placeholder='Input the amount of tokens you wnat to purchase' />
    <button className='block mx-auto bg-black/40 border-2 text-cyan-500 w-full h-8 rounded-full hover:border-cyan-300 hover:text-cyan-500/60 border-cyan-500' onClick={props.buy}>Buy Tokens</button>
    </div>
